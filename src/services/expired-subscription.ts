  ...
  try {
    ...
    if (user.telegramChatId) {
      try {
        ...
        console.error(`Failed to notify user ${user.id} about subscription expiration:`, notifyError);
      }
    }
  } catch (error) {
...
async function notifyUserOfExpiration(user: User): Promise<void> {
  ...
  try {
    ...
    const messageKey = language === 'RU' 
      ? 'bot:subscription.expired_ru' 
      : 'bot:subscription.expired_en';
    ...
    await bot.telegram.sendMessage(user.telegramChatId, message, {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: '🔄 Renew Subscription', callback_data: 'subscribe' }]
        ]
      }
    });
